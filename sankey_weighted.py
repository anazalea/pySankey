...
Copyright (C) 2016  Marco Manzoni & Anneya Golob
"""
...
def sankey_weighted(left,right,leftWeight=[],rightWeight=[],colorDict={},leftLabels=[],rightLabels=[],aspect=4,rightColor=False):
    '''
    ...
    Inputs:
        ...
        right = NumPy array of corresponding labels on the right of the diagram 
            len(right) == len(left)
        leftWeight = NumPy array of weights for each strip starting from the left of the diagram, if not specified 1 is assigned
        rightWeight = NumPy array of weights for each strip starting from the right of the diagram, if not specified the corresponding leftWeight is assigned
    ...
    #Create Dataframe    
    df = pd.DataFrame({'left':left,'right':right,'leftWeight':leftWeight,'rightWeight':rightWeight},index=range(len(left)))
    ...
    for l in leftLabels:
        plt.fill_between([-0.02*xMax,0],2*[widths_left[l]['bottom']],\
            2*[widths_left[l]['bottom']+widths_left[l]['left']],color=colorDict[l],alpha=0.99)
        plt.text(-0.05*xMax,widths_left[l]['bottom']+0.5*widths_left[l]['left'],l,{'ha': 'right', 'va': 'center'}, fontsize=20)
    for l in rightLabels:
        plt.fill_between([xMax,1.02*xMax],2*[widths_right[l]['bottom']],\
            2*[widths_right[l]['bottom']+widths_right[l]['right']],color=colorDict[l],alpha=0.99)
        plt.text(1.05*xMax,widths_right[l]['bottom']+0.5*widths_right[l]['right'],l,{'ha': 'left', 'va': 'center'}, fontsize=20)
    ...
    for l in leftLabels:
        for l2 in rightLabels:
            ...
            if len(df[(df.left==l) & (df.right==l2)]) > 0:
                ...
                plt.fill_between(np.linspace(0,xMax,len(ys_d)),ys_d,ys_u,alpha=0.65,color=colorDict[lc])
